  ...
  class CliInterface
    ...
    def self.list_databases
      ...
      Dir["#{StaticConfigurations::DataDirectory}*"].each do |db|
        puts format('[%8d bytes] %s', File.size(db), db)
      end
    end
    ...
    def focus(cmd)
      ...
      IssueUi.new(issue).run if issue
    ...
    def print_help
      ['new', 'Create a new log entry',
       ...
       'config', 'Set differeing configuration parameters'].each_with_index do |el, ix|
        print '  ' if 1 == ix % 2
        puts el
      end
    end
    ...
    def print_date_collection(stat, issues)
      print @strmaker.green(stat.to_s)
